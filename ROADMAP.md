...
- [~] **Plugins + marketplace** — local plugins shipped; marketplace pending.
  ...
  - [ ] marketplace/install (network, versioning, `/plugin install`) — the deferred heavy half
  - [ ] plugin-contributed MCP servers; live reload of plugin *commands* (today needs a restart)
- [~] **Workflows** — declarative engine shipped; dynamic half pending.
  ...
  - [ ] **dynamic** orchestration / `/ultraplan` — agent composes a workflow on the fly (needs a scripting runtime)
  - [ ] concurrency cap + progress streaming of sub-agent output (today: per-stage ticks + final result)
