...
export function toolListToSystemPrompt(tools: Tool[]): string {
  ...
  const lines = [
    ...
    "- Keep working until the task is actually DONE — don't stop after one step to wait for the user. Above all: if you say you'll do something ('I'll check X', 'let me look at Y', 'next I'll run Z'), CALL THE TOOL in that SAME turn. Never announce an action in prose and then end your turn — that just makes the user type 'continue'. Act first, narrate after. Stop only when the task is complete or you genuinely need input you can't get yourself.",
